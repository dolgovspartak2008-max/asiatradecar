...
export default async function CarPage({ params }: Props) {
  ...
  return <section className="page-section car-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema).replace(/</g, "\\u003c") }}/><div className="container"><a className="back-link" href="/catalog?country=kr">← В каталог</a><div className="car-detail-grid"><div><div className="car-gallery">{car.photos.length ? car.photos.slice(0, 5).map((photo, index) => <div className={index === 0 ? "main-photo" : ""} key={photo}><Image src={photo} alt={`${name}, фото ${index + 1}`} fill sizes={index === 0 ? "(max-width: 900px) 100vw, 60vw" : "30vw"} priority={index === 0} unoptimized /></div>) : <div className="detail-placeholder"><Icon name="car" size={64}/><span>Фотографии обновляются из источника</span></div>}</div><div className="car-specs"><h2>Характеристики</h2><dl>{specs.map(([label, value]) => <div key={String(label)}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></div></div><aside className="car-sidebar"><p className="eyebrow">В наличии у источника</p><h1>{name}</h1><p className="detail-price">{car.priceRub ? formatRub(car.priceRub) : formatKrw(car.priceKrw)}</p>{car.priceRub && <p>{formatKrw(car.priceKrw)} — цена автомобиля без итоговых расходов</p>}<div className="notice"><Icon name="shield"/><p>Перед выкупом автомобиль и документы проходят отдельную проверку. Итоговая стоимость фиксируется после расчёта.</p></div><LeadForm formId="car-detail" compact carName={name}/></aside></div></div></section>;
}

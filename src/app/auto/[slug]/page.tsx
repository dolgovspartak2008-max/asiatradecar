...
export default async function CarPage({ params }: Props) {
  ...
  return <section className="page-section car-page">
    ...
    <div className="container"><div className="car-detail-grid">
      ...
      <aside className="car-sidebar">{sourceLink && <a className="source-car-button" href={sourceLink.href} target="_blank" rel="noreferrer">{sourceLink.label} <Icon name="arrow" /></a>}<p className="detail-price">{car.priceRub ? formatRub(car.priceRub) : "Цена уточняется"}</p><p className="price-caption">{car.country === "kr" ? "Под ключ в РФ" : "Предварительный расчёт для РФ"}</p><div className="car-detail-actions"><PriceBreakdown slug={car.slug} carName={name} priceKrw={car.priceKrw} priceRub={car.priceRub} details={car.details} currencyCode={car.currencyCode} country={car.country} /><ApplicationDialog carName={name} /></div><div className="notice"><Icon name="shield"/><p>История и расчёт загружены из источника. Перед выкупом автомобиль и документы проверяются отдельно.</p></div></aside>

...
export default function OrdersPage() {
  return <>
    ...
    <section className="section orders-control"><div className="container orders-control-grid"><div><h2>Что остаётся под контролем</h2><p>До оплаты вы видите автомобиль, проверку и расчёт. По ходу доставки получаете документы по пройденным этапам.</p><ul>{documents.map((item) => <li key={item}><Icon name="check"/><span>{item}</span></li>)}</ul></div><div className="orders-cost"><h3>Из чего складывается стоимость</h3><dl><div><dt>Автомобиль</dt><dd>цена лота</dd></div><div><dt>Проверка и выкуп</dt><dd>по договору</dd></div><div><dt>Логистика</dt><dd>Корея, море, Россия</dd></div><div><dt>Таможня</dt><dd>по параметрам авто</dd></div><div><dt>Доставка до города</dt><dd>по маршруту</dd></div></dl><Link className="text-link" href="/calculator">Открыть калькулятор <Icon name="arrow"/></Link></div></div></section>

...
export async function POST(request: NextRequest) {
  ...
  const localRequest = ["localhost", "127.0.0.1", "[::1]"].includes(request.nextUrl.hostname);
  if (!operatorReady || !retentionReady || (!databaseReady && !localRequest) || !process.env.TELEGRAM_BOT_TOKEN || !process.env.TELEGRAM_CHAT_ID) {
    return unavailable();
  }
  if (databaseReady) {
    try {
  ...
  try {
    ...
    if (leadId) await query("UPDATE leads SET delivery_status='sent' WHERE id=$1", [leadId]).catch(() => undefined);
    return NextResponse.json({ saved: true, message: "Заявка отправлена. Мы свяжемся с вами после обработки запроса." });
  } catch {
    if (!leadId) return unavailable();

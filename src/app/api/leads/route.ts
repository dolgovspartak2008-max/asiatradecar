...
export async function POST(request: NextRequest) {
  ...
  if (!operatorReady || !retentionReady || !hasDatabase() || !process.env.TELEGRAM_BOT_TOKEN || !process.env.TELEGRAM_CHAT_ID) {
    return NextResponse.json({ message: "Канал заявок ещё не подключён. Форма не сохраняет ваши данные." }, { status: 503 });
  }
  ...
  const leadId = await inTransaction(async (client) => {
  ...
  try {
    ...
    await query("UPDATE leads SET delivery_status='sent' WHERE id=$1", [leadId]);
    return NextResponse.json({ message: "Заявка отправлена. Мы свяжемся с вами после обработки запроса." });
  } catch {
    await query("UPDATE leads SET delivery_status='failed' WHERE id=$1", [leadId]);
    return NextResponse.json({ message: "Заявка сохранена, но канал уведомлений временно недоступен. Попробуйте позже." }, { status: 502 });
  }
}

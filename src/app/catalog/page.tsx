...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const page = Math.floor(filters.offset / filters.limit) + 1; const pages = Math.ceil(total / filters.limit);
  return <section className="page-section"><div className="container"><div className="catalog-top"><div><p className="eyebrow">Южная Корея</p><h1>Каталог автомобилей</h1><p>{sourceError ? "Live-источник временно не отвечает" : `${total.toLocaleString("ru-RU")} предложений в live-каталоге`}</p></div><Link className="button button-ghost" href="/catalog/favorites"><Icon name="heart" /> Избранное</Link></div>
    ...
    {cars.length ? <><div className="catalog-grid">{cars.map((car) => <CarCard key={car.id} car={car} />)}</div>{pages > 1 && <nav className="pagination" aria-label="Страницы каталога">{page > 1 && <Link href={{ query: { ...raw, page: page - 1 } }}>Назад</Link>}<span>{page} / {pages}</span>{page < pages && <Link href={{ query: { ...raw, page: page + 1 } }}>Далее</Link>}</nav>}</> : <div className="empty-state"><Icon name="car" size={48}/><h2>{sourceError ? "Каталог временно недоступен" : "Автомобили не найдены"}</h2><p>{sourceError ? "Источник не ответил вовремя. Обновите страницу через минуту." : "Измените фильтры — live-каталог обновится автоматически."}</p><Link className="button" href="/catalog?country=kr">{sourceError ? "Обновить каталог" : "Сбросить фильтры"}</Link></div>}
  </div></section>;
}

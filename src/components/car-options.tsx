...
export function CarOptions({ details }: { details: Record<string, unknown> }) {
  ...
  return <section className="car-options" aria-labelledby="options-title"><p className="eyebrow">Данные источника</p><h2 id="options-title">Комплектация и данные лота</h2><div>{groups.map((group) => <details key={group.title}><summary><span>{group.title}</span><b>{group.items.length}</b></summary><ul>{group.items.map((item, index) => <li key={`${item}-${index}`}>{item}</li>)}</ul></details>)}</div></section>;
}

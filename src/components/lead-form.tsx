...
export function LeadForm({ formId, compact = false, carName, calculationRub, cars, catalogSearch = false }: Props) {
  ...
  return <form className={`lead-form ${compact ? "lead-form-compact" : ""}`} onSubmit={submit} noValidate>
    ...
    {catalogSearch && <label>Автомобиль из каталога<input name="selectedCar" list={`${formId}-catalog-options`} value={catalogQuery} onChange={(event) => { const value = event.target.value; setCatalogQuery(value); if (value.trim().length < 2) setCatalogMatches([]); }} placeholder="Начните вводить марку или модель" autoComplete="off"/><datalist id={`${formId}-catalog-options`}>{catalogMatches.map((car) => <option value={car.label} key={car.slug}/>)}</datalist><small>Поиск идёт по всему синхронизированному каталогу Южной Кореи. Можно оставить поле пустым.</small></label>}

...
export function CarGallery({ photos, name }: { photos: string[]; name: string }) {
  ...
  return <div className="car-gallery">
    <div className="main-photo"><Image src={availablePhotos[active]} alt={`${name}, фото ${active + 1}`} fill sizes="(max-width: 900px) 100vw, 64vw" loading="eager" onError={() => removePhoto(availablePhotos[active])} /></div>

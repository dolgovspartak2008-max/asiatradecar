...
export function CarGallery({ photos, name }: { photos: string[]; name: string }) {
  ...
  return <div className="car-gallery">
    <button className="main-photo" type="button" onClick={() => dialog.current?.showModal()} aria-label={`Открыть фото ${active + 1} на весь экран`}><Image src={availablePhotos[active]} alt={`${name}, фото ${active + 1}`} fill sizes="(max-width: 900px) 100vw, 64vw" loading="eager" onError={() => removePhoto(availablePhotos[active])} /></button>
    <div className="car-thumbnails" aria-label={`Фотографии ${name}`}>{availablePhotos.map((photo, index) => <button className={index === active ? "active" : ""} type="button" onClick={() => setActive(index)} aria-label={`Показать фото ${index + 1}`} aria-pressed={index === active} key={photo}><Image src={photo} alt="" fill sizes="120px" onError={() => removePhoto(photo)} /></button>)}</div>
    <dialog ref={dialog} className="car-photo-dialog" onClick={(event) => { if (event.target === event.currentTarget) event.currentTarget.close(); }}>
      ...
      <div><Image src={availablePhotos[active]} alt={`${name}, фото ${active + 1}`} fill sizes="100vw" loading="eager" onError={() => { dialog.current?.close(); removePhoto(availablePhotos[active]); }} /></div>
    </dialog>
  </div>;
}

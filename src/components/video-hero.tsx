...
export function VideoHero() {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      else { void ref.current.play().then(() => setPlaying(true)).catch(() => setPlaying(false)); }
    };
    sync();
  ...
  return <div className={`hero-media ${playing ? "is-playing" : ""}`} aria-label="Автомобиль едет по горной дороге">
    <video ref={ref} autoPlay muted loop playsInline preload="metadata" poster="/media/hero-import.png"><source src="/media/hero-drive.mp4" type="video/mp4" />Ваш браузер не поддерживает фоновое видео.</video>

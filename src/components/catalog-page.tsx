...
export async function CatalogPageContent({ market, raw }: { market: CatalogMarket; raw: SearchParams }) {
  ...
  const result = await getCatalog(filters).then((data) => ({ data, sourceError: false })).catch(() => ({ data: { cars: [], total: 0, makes: [], models: [] }, sourceError: true }));
  const { data: { cars, total, makes, models }, sourceError } = result;
  rememberCatalogPrices(cars);

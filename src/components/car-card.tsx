...
export function CarCard({ car, onOpen }: { car: FavoriteCar; onOpen?: () => void }) {
  ...
  return <article className="car-card">
    ...
    <div className="car-card-body"><div className="car-card-meta"><p className="eyebrow">{meta}</p>{insurance && <span className="car-insurance-summary"><Icon name="collision" size={14} />{insurance}</span>}</div><h2><Link href={href} prefetch={false} onClick={onOpen}>{name}</Link></h2><div className="car-chips">{car.engineCc && <span>{(car.engineCc / 1000).toFixed(1)} л</span>}{car.powerHp && <span>{car.powerHp} л.с.</span>}{car.country === "kr" && car.fuel && <span>{car.fuel}</span>}{car.drive && <span>{car.drive}</span>}</div><div className="car-price-row"><div><span>Под ключ в РФ</span><p className="car-price">{car.priceRub ? formatRub(car.priceRub) : "Цена уточняется"}</p></div></div></div>
  </article>;
}

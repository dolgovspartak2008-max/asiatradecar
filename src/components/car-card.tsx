...
export function CarCard({ car }: { car: FavoriteCar }) {
  ...
  return <article className="car-card">
    <Link className="car-image" href={href} aria-label={`Открыть ${name}`} prefetch={false}>
      {car.photos[0] ? <Image src={car.photos[0]} alt={name} fill sizes="(max-width: 720px) 100vw, 33vw" /> : <div className="car-placeholder"><Icon name="car" size={42} /><span>Фото обновляется</span></div>}
    </Link>

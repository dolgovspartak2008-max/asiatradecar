...
export function CarCard({ car, onOpen }: { car: FavoriteCar; onOpen?: () => void }) {
  ...
  return <article className="car-card">
    <Link className="car-image" href={href} aria-label={`Открыть ${name}`} prefetch={false} onClick={onOpen}>
      {photo && !imageFailed ? <Image src={photo} alt={name} fill sizes="(max-width: 720px) 100vw, 33vw" unoptimized onError={() => setImageFailed(true)} /> : <div className="car-placeholder"><Icon name="car" size={42} /><span>Фото обновляется</span></div>}
    </Link>

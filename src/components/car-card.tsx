...
export function CarCard({ car }: { car: Car }) {
  ...
  return <article className="car-card">
    ...
    <div className="car-card-body"><p className="eyebrow">{car.year} · {car.mileageKm.toLocaleString("ru-RU")} км</p><h2><Link href={href} prefetch={false} target={external ? "_blank" : undefined} rel={external ? "noreferrer" : undefined}>{name}</Link></h2><div className="car-chips">{car.engineCc && <span>{(car.engineCc / 1000).toFixed(1)} л</span>}{car.powerHp && <span>{car.powerHp} л.с.</span>}{car.drive && <span>{car.drive}</span>}</div><div className="car-price-row"><div><span>Под ключ в РФ</span><p className="car-price">{car.priceRub ? formatRub(car.priceRub) : "Цена уточняется"}</p></div></div></div>
  </article>;
}

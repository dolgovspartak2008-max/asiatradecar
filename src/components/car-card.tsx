...
export function CarCard({ car }: { car: Car }) {
  ...
  return <article className="car-card">
    <Link className="car-image" href={`/auto/${car.slug}`} aria-label={`Открыть ${name}`}>
    ...
    <div className="car-card-body"><p className="eyebrow">{car.year} · {car.mileageKm.toLocaleString("ru-RU")} км</p><h2><Link href={`/auto/${car.slug}`}>{name}</Link></h2><div className="car-chips">{car.engineCc && <span>{(car.engineCc / 1000).toFixed(1)} л</span>}{car.powerHp && <span>{car.powerHp} л.с.</span>}{car.drive && <span>{car.drive}</span>}</div><p className="car-price">{car.priceRub ? formatRub(car.priceRub) : formatKrw(car.priceKrw)}</p>{car.priceRub && <small>{formatKrw(car.priceKrw)} · без расходов по РФ</small>}</div>
  </article>;
}

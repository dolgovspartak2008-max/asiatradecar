...
export function CarCard({ car }: { car: FavoriteCar }) {
  ...
  return <article className="car-card">
    ...
    <div className="car-card-body"><div className="car-card-meta"><p className="eyebrow">{meta}</p>{insurance && <span className="car-insurance-summary">{insurance}</span>}</div><h2><Link href={href} prefetch={false}>{name}</Link></h2><div className="car-chips">{car.engineCc && <span>{(car.engineCc / 1000).toFixed(1)} л</span>}{car.powerHp && <span>{car.powerHp} л.с.</span>}{car.drive && <span>{car.drive}</span>}</div><div className="car-price-row"><div><span>Под ключ в РФ</span><p className="car-price">{car.priceRub ? formatRub(car.priceRub) : "Цена уточняется"}</p></div></div></div>
  </article>;
}

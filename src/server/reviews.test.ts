...
it("shows all 29 imported reviews when the local database is not configured", async () => {
  vi.mocked(hasDatabase).mockReturnValue(false);

  const reviews = await getPublishedReviews();

  expect(reviews).toHaveLength(29);
  expect(new Set(reviews.map((review) => review.id)).size).toBe(29);
...
it("keeps reviews visible during a temporary database failure", async () => {
  ...
  await expect(getPublishedReviews()).resolves.toHaveLength(29);
});
